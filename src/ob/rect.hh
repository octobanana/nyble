...
public:
  ...
  Rect& draw(std::ostream& os)
  {
    ...
    for (std::size_t y = 0; y < _h && y + _y <= _y_max; ++y)
    {
      ...
      for (std::size_t x = 0; x < _w && x + _x <= _x_max; ++x)
      {
        ...
        {
          if (_fill == " " && (_color_bg.key() == "clear" || _color_bg.key().empty()))
          {
            ++cursor_right;
          }
          else
  ...
  Rect& border_fill(std::string const& str)
  {
    _line_top = str;
    _line_right = str;
    _line_bottom = str;
    _line_left = str;
    _corner_top_left = str;
    _corner_top_right = str;
    _corner_bottom_right = str;
    _corner_bottom_left = str;

    return *this;
  }

  Rect& border_lines(std::string const& top, std::string const& right, std::string const& bottom, std::string const& left)
  {
    _line_top = top;
    _line_right = right;
    _line_bottom = bottom;
  ...
  Rect& border_corners(std::string const& top_left, std::string const& top_right, std::string const& bottom_right, std::string const& bottom_left)
  {
